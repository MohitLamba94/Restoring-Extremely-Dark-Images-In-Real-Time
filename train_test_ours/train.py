...
opt['iterations'] = 100005  # The model will run for these many iterations.
dry_run = False  # If you wish to first test the entire workflow, for couple of iterations, make this TRUE
# If dry run flag is set TRUE the code will terminate after these many iterations
...
# Intermediate details for the train images, such as estimated amplification will be saved here.
...
for x in train_files:
    gt_files += glob.glob('SID_cvpr_18_dataset/Sony/long/*' +
                          x[-17:-12]+'*.ARW')

dataloader_train = DataLoader(load_data(train_files, gt_files, train_amplification_file, 20, gt_amp=True,
                              training=True), batch_size=opt['batch_size'], shuffle=True, num_workers=0, pin_memory=True)
...
for x in test_files:
    gt_files = gt_files + \
        glob.glob('SID_cvpr_18_dataset/Sony/long/*'+x[-17:-12]+'*.ARW')
...
for i, img in enumerate(dataloader_train):
    print('Input image size : {}, GT image size : {}'.format(
        img[0].size(), img[1].size()))
    break
...
while iter_num < opt['iterations']:
    for _, img in enumerate(dataloader_train):
        ...
        if iter_num % 10 == 0:
            print(iter_num)
            if iter_num % 100 == 0:
                loss_list.append('{},{},{}'.format(
                    loss1.item(), -1, loss3.item()))
        ...
        if iter_num in opt['atWhichSave']:
            ...
            if iter_num == opt['atWhichSave'][0]:
                ...
                mode = 'a'
            run_test(model, dataloader_test, iter_num, save_images,
                     save_csv_files, metric_average_file, mode, training=True)
            torch.save({'model': model.state_dict()}, os.path.join(
                save_weights, 'weights_{}'.format(iter_num)))
...
np.savetxt(os.path.join(save_csv_files, 'loss_curve.csv'), [
           p for p in zip(loss_iter_list, loss_list, iter_LR)], delimiter=',', fmt='%s')
